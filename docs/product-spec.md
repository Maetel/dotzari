...
| 2026-08-03 | D-022 | 모바일 Viewer는 최초 렌더 뒤 실제 캔버스 폭이 안정화될 때까지 현재 스텝 카메라를 새 폭으로 다시 계산한다. 사용자가 캔버스를 드래그하거나 확대·축소하면 카메라 위치를 그대로 보존하고, 이후 viewport 변화로 자동 재정렬하지 않는다. 스텝이 바뀌면 새 스텝에 한해 자동 초점을 다시 허용한다. | iOS Safari는 브라우저 UI와 layout viewport가 정리되는 동안 캔버스 폭이 달라질 수 있다. 최초 폭에서 한 번만 초점을 맞추면 최종 폭에서 노드가 한쪽으로 밀리지만, 모든 크기 변경마다 초점을 맞추면 사용자가 놓은 위치를 빼앗기 때문이다. |
| 2026-08-04 | D-023 | 모바일 카메라는 캔버스 element의 전체 `clientWidth`가 아니라 캔버스 content rect와 `visualViewport`가 실제로 겹치는 영역을 기준으로 현재 노드를 배치한다. 브라우저 확대 상태에서도 같은 월드 좌표와 캔버스 확대 비율을 유지하며, 보이는 영역의 중심만 카메라 계산에 반영한다. | iPhone Safari에서 브라우저 확대 또는 복원된 시각 영역 배율이 적용되면 layout viewport보다 실제 보이는 폭이 작다. 전체 `clientWidth`의 중앙에 맞추면 현재 노드가 화면 오른쪽으로 밀리기 때문이다. |
...
26. iOS Safari를 모사해 같은 페이지의 캔버스 폭을 590px에서 390px로 바꿀 때 사용자 조작 전에는 현재 스텝을 다시 맞추고, 드래그·확대·축소 뒤에는 기존 카메라를 유지함
27. 390×844에서 브라우저 배율 100%와 130%를 각각 적용할 때 현재 스텝과 다음 스텝 노드가 `visualViewport`와 캔버스의 교차 영역 중앙에 있고 문서 가로 넘침이 없음
