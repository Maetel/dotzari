...
| Q-014 | 축소 시 묶음의 소속을 부모·자식 관계, 연결 관계, 태그, 제작자 지정 중 무엇으로 결정하고 줌 임계값을 어떻게 설정하는가? | 디자인 탐색 후 결정 |
...
줌 수준별 묶음·상세 표현을, D-015에 따라 하위 노드 표현과 조합 방식을 함께 실험한다. 두 테마는 현행 탐색 기준으로 유지한다.
...
- 부모 노드는 npm처럼 의미상 소속된 하위 노드를 안에 포함해 계층을 표현할 수 있어야 한다.
...
- 묶음 소속 결정 방식과 실제 줌 임계값은 Q-014의 미결정 사항이며 현행 수치는 디자인 검증용이다.
...
사용자에게 노출하는 콘텐츠 계층은 PowerPoint·Keynote의 익숙한 개념을 따른다.

- **강의(Lesson):** 하나의 학습 또는 발표 단위다. 여러 슬라이드로 구성된다.
- **슬라이드(Slide):** 내용이 긴밀하게 연결된 하나의 설명 단위다. 사용자가 처음 “키프레임”이라고 부른 여러 프레임의 집합에 해당하며, 하나 이상의 단계를 가진다.
- **단계(Step):** 슬라이드 안에서 한 번의 설명 진행 후 도달하는 개별 표현 상태다. 노드, 엣지, viewport, 콘텐츠 상태를 포함한다.
- **단계 애니메이션:** 현재 단계에서 다음 단계로 진행할 때 발생하는 노드·엣지·카메라 변화다.
- **슬라이드 전환:** 현재 슬라이드의 마지막 단계에서 다음 슬라이드로 이동하는 전환이다.

저작 UI는 사용자가 슬라이드와 단계를 다루도록 하고, `keyframe`이라는 단어를 노출하지 않는다. 저수준 엔진에서는 시간에 따른 개별 속성값을 나타내는 일반적인 애니메이션 keyframe 용어를 사용할 수 있다.

기존 초기 사양의 `Scene`은 현재 사용자 모델의 `Step`에 가장 가깝다. 새 구현은 `Slide`와 그 안의 ordered steps를 표현할 수 있어야 하며, 본문에 남아 있는 `DiagramScene` 등의 이름은 최종 public API가 아니라 초기 초안으로 취급한다.
...
노드 내부 Tooltip이 캔버스 transform이나 overflow에 잘리지 않도록 `PortalLayer`를 제공한다.
