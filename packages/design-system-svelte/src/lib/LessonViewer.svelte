...
<section class="dz-lesson-viewer" aria-label={lesson.title}>
  ...
  <div class="dz-lesson-viewer__explanation" aria-live="polite">
    <strong>{selectedNode?.title ?? snapshot.step.title}</strong>
    <p>{selectedNode?.description ?? snapshot.step.description}</p>
  </div>
...
<style>
  .dz-lesson-viewer { display: grid; grid-template-rows: auto auto auto minmax(420px, 1fr) auto auto; gap: var(--dz-space-3); width: 100%; min-height: 760px; padding: var(--dz-space-4); overflow: hidden; border: 1px solid var(--dz-border-strong); border-radius: var(--dz-radius-xl); background: var(--dz-surface-panel); color: var(--dz-text-primary); box-shadow: var(--dz-shadow-panel); }
  ...
  @media (max-width: 620px) { .dz-lesson-viewer { min-height: 720px; padding: var(--dz-space-3); border-radius: var(--dz-radius-lg); }.dz-lesson-viewer__copy h2 { font-size: 1.8rem; }.dz-lesson-viewer__canvas { min-height: 440px; } }
</style>
